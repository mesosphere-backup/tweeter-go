...
# Tests Tweeter's usage of Cassandra.
...
for i in $(seq 1 ${NUM_OINKS}); do
  ...
  HTML="$(curl --fail --silent --show-error -X POST -d "handle=${HANDLE}&content=${CONTENT}" http://tweeter.acme.org/oink)"
